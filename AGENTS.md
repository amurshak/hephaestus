...
## Repo Shape
- This is a shell/prose workflow repo, not a package-manager project; there is no `package.json`. CI is `.github/workflows/tests.yml` (runs `tests/run.sh`, Claude/OpenCode/Codex adapter drift checks, and the composition check).
...
- Adapter changes usually need `./scripts/sync-agent-adapters.sh --check`, `./scripts/sync-opencode-adapters.sh --check`, `./scripts/sync-codex-adapters.sh --check`, and `./tests/run.sh`; workflow-command drift is a common failure mode.
