---
description: "Orient in the current project. On first contact with an unprepared project, set up the workflow's operating requirements, then orient. Run autonomously."
---
<!-- requires: none -->
<!-- chains: /worktrees -->
<!-- generated from .ai/workflows/orient.md; do not edit directly -->

> **OpenCode:** start from the project root that contains `.opencode/`. Invoke nested workflows as slash commands (/worktrees) so their full templates load — do not paraphrase. Spawn role agents with the Task tool or @mentions.
