...
**Always delegate to agent(s) (Task tool or @mention)** — test output is verbose and should not pollute the main context window.
...
3. **Launch @tester agent(s)**:
   - Check CLAUDE.md to determine the correct test and lint commands for this project
   - Launch one or more @tester agents based on changed areas (parallelize if there are independent test suites)
...
5. **Report results** (concise — the agents (Task tool or @mention) already absorbed the verbose output):
