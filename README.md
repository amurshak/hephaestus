...
If `changelog.d/` already holds another tool's fragments — [scriv](https://scriv.readthedocs.io/) claims the same directory by default — adoption proceeds with a warning: the files stay untouched, but `collect-changelog.sh --check` will reject any name other than `<id>.<category>.md`. There is no automatic rename, because a scriv fragment carries its category inside the body rather than the filename, and one fragment can hold several — there is no name to rename to. Migrate by folding the pending fragments with the tool that wrote them (`scriv collect` empties the directory), then adopt; or rewrite each by hand as `<id>.<category>.md`, its body the entry text without the leading `- `. Fragments from a towncrier setup already shaped `<issue>.<type>.md` only need their type renamed to `added`, `changed`, `fixed`, or `removed`.
