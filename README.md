...
**Within a session**: multiple coder agents work in parallel worktrees for independent tasks; multiple explorer and researcher agents fan out across subsystems simultaneously. **Across issues**: `/worktrees` surveys and reaps finished worktrees, plans a wave of mutually non-conflicting issues under a configurable cap (declare contention hotspots in a `## Worktrees` section of CLAUDE.md), creates a sibling worktree per issue with config propagated and env files single-sourced, and spawns a seeded agent session in each. A spawned session cannot remove the worktree it occupies, so its `/finish` leaves the worktree and branch behind; `/orient` chains `/worktrees cleanup` from the primary checkout to collect them. **Across time**: `loop.sh` runs `/autopilot` in fresh sessions on a timer, each picking up the next issue.
...
The first command installs the shared commands, agents, and skills into your harness config dirs — `~/.claude/`, `~/.config/opencode/`, `~/.codex/`, `~/.hermes/`, `~/.cursor/` — so they work in **every** project. The second scaffolds the handful of files a project owns: `orient` for each harness, `AGENTS.md`, `opencode.json`, `.hermes/.gitignore`. Commands run under bare names (`/autopilot`, `/ship`). `~/.hephaestus/update.sh` pulls and re-links. Full detail in [Script install](#script-install-every-project-at-once).
...
`install.sh` links all three families into `~/.cursor/` (honoring `$CURSOR_HOME`) for every project; a vendored or hand-copied `.cursor/` works too. All are generated by `scripts/sync-cursor-adapters.sh`:
...
5. Role work uses `delegate_task`. Each brief in `.hermes/agents/` gives the `toolsets` to pass (`["terminal", "file"]` for coder, `["file", "web"]` for researcher) and the prompt to send as `context`. Three Hermes specifics: a delegate inherits **none** of the parent conversation, and the cwd it does inherit is frozen at session launch — confidently stale in a worktree — so `context` carries absolute paths and everything else it needs; the `toolsets` you pass are intersected with the session's and anything missing is dropped silently, so a researcher delegate in a session without `web` loses web search with no error; and `delegate_task` has no per-child worktree, so parallel coder delegates share one working tree — serialize file-modifying work.
...
Symlinks the shared commands, agents, and skills into `~/.claude/`, `~/.config/opencode/`, `~/.codex/`, `~/.hermes/`, and `~/.cursor/` (honoring `$CLAUDE_CONFIG_DIR`, `$XDG_CONFIG_HOME`, `$CODEX_HOME`, `$HERMES_HOME`, and `$CURSOR_HOME`), then runs a health check. Commands work under bare names (`/autopilot`, `/ship`, …) in every project — no plugin namespace, and for Hermes no `external_dirs` wiring.
...
If `changelog.d/` already holds another tool's fragments — [scriv](https://scriv.readthedocs.io/) claims the same directory by default — adoption proceeds, leaves them untouched, and warns on every run until the directory folds clean. There is no automatic rename: a scriv fragment carries its category inside the body rather than the filename, and one fragment can hold several, so no `<id>.<category>.md` name exists to rename to. Nor can the two conventions coexist: `collect-changelog.sh` folds only `<id>.<category>.md` — `--check` rejects other `.md` names and never inspects the rest, so scriv's `.rst` default sits unfolded silently rather than failing loudly. Migrate by folding the pending fragments with the tool that wrote them (`scriv collect` empties the directory), or rewrite each by hand as `<id>.<category>.md`, its body the entry text without the leading `- `. Towncrier-style `<issue>.<type>.md` fragments only need their type renamed to `added`, `changed`, `fixed`, or `removed`.
