...
### Added
- **Hook templates** (#114): `templates/hooks/protect-files.sh` (blocks Edit/Write to `.env*`, `.git/` internals, and path-traversal escapes; works as-is) and `templates/hooks/lint-on-commit.sh` (blocks `git commit` while lint fails; set `LINT_CMD`). Deterministic backstops for the prose quality gates, adapted from the RWD lineage; README documents the curl install and settings wiring.
