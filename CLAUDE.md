...
- **Codex adapters are generated too**: `.agents/skills/*/SKILL.md` comes from `.ai/workflows/`; `.codex/agents/*.toml` comes from `.claude/agents/`. The generator localizes the same Claude dialect, including the `/worktrees` spawn CLI → `codex "<prompt>"`. Codex reads skills from `$CODEX_HOME/skills` (user install) and the project's `.agents/skills` (project install), and matches `/start-issue <N>` to a skill implicitly via its `Use for /<name> requests.` description anchor — `bash scripts/verify-codex-load.sh` checks both against a live CLI. Run `./scripts/sync-codex-adapters.sh` after workflow or agent edits and `./scripts/sync-codex-adapters.sh --check` in quality gates.
