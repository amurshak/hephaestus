...
- **orient is project-specific** — it is excluded from symlinking in every harness. `install.sh --project` scaffolds a template from `templates/orient.md` that must be customized. The shipped generic `/orient` covers install paths without install.sh (plugin, manual copy): on first run in an unprepared project it bootstraps the operating requirements — infers a Development Commands section from manifests and scaffolds a project orient — additively, never overwriting. It skips that bootstrap in this repo, detected by `.ai/workflows/` plus `scripts/sync-agent-adapters.sh`: `.claude/commands/orient.md` here is the generated consumer adapter, and without the guard it offers to infer a Development Commands section into the codebase that defines what one is.
