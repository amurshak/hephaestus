...
At release, `scripts/collect-changelog.sh <version>` folds every fragment into a dated `## <version>` section and deletes them (`--preview` to dry-run, `--check` to validate filenames). The fold reads the working tree, so deletion forces `git rm -f` — the on-disk content is exactly what published — and a release dies rather than claim success over a surviving fragment or an already-released version (#205). Hand-written `## Unreleased` content is merged by category, so the transition loses nothing — and `--preview` renders that merge too, reporting fragment and legacy-entry counts separately, so the rehearsal shows exactly what the fold will publish (#206). `.gitattributes` sets `CHANGELOG.md merge=union` as a backstop for any direct edit that slips through.
...
- `.ai/agents/` — canonical subagent definitions (coder, reviewer, tester, explorer, researcher) with tool permissions and structured output contracts
...
- `.claude/agents/` — generated Claude subagent adapters from `.ai/agents/`; still Claude Code's discovery path, committed like every other render target
- `.opencode/commands/` — generated OpenCode command adapters for the same canonical workflows
- `.opencode/agents/` — generated OpenCode subagent adapters from `.ai/agents/`
- `.agents/skills/` — generated Codex skill adapters for the same canonical workflows
- `.codex/agents/` — generated Codex agent-role adapters from `.ai/agents/`
- `.hermes/skills/hephaestus/` — generated Hermes skill adapters for the same canonical workflows
- `.hermes/agents/` — generated Hermes `delegate_task` briefs from `.ai/agents/`
...
- `scripts/sync-agent-adapters.sh` — generates/checks Claude command adapters from `.ai/workflows/` and Claude agent adapters from `.ai/agents/`
...
- **Workflows live in `.ai/workflows/` and agent definitions in `.ai/agents/`** as the canonical sources. `.claude/commands/` and `.claude/agents/` files are generated adapters; run `./scripts/sync-agent-adapters.sh` after workflow or agent edits and `./scripts/sync-agent-adapters.sh --check` in quality gates.
- **OpenCode adapters are generated too**: `.opencode/commands/` comes from `.ai/workflows/`; `.opencode/agents/` comes from `.ai/agents/`. The generator localizes Claude dialect (worktrees/subagents/TodoWrite → Task/`@agent` + serialize edits, and the `/worktrees` spawn CLI → `opencode --prompt`) and injects slash-command chain notes. Run `./scripts/sync-opencode-adapters.sh` after workflow or agent edits and `./scripts/sync-opencode-adapters.sh --check` in quality gates. Verify a live open session loads them with `bash scripts/verify-opencode-load.sh`.
- **Codex adapters are generated too**: `.agents/skills/*/SKILL.md` comes from `.ai/workflows/`; `.codex/agents/*.toml` comes from `.ai/agents/`. The generator localizes the same Claude dialect, including the `/worktrees` spawn CLI → `codex "<prompt>"`. Codex reads skills from `$CODEX_HOME/skills` and a project's own `.agents/skills`, and matches `/start-issue <N>` to a skill implicitly via its `Use for /<name> requests.` description anchor (both measured against codex-cli 0.145.0). `bash scripts/verify-codex-load.sh` guards the spawn form live — that `codex --help` still takes a positional prompt — and checks the adapters are in a root Codex reads. Run `./scripts/sync-codex-adapters.sh` after workflow or agent edits and `./scripts/sync-codex-adapters.sh --check` in quality gates.
- **Hermes adapters are generated too**: `.hermes/skills/hephaestus/*/SKILL.md` comes from `.ai/workflows/`; `.hermes/agents/*.md` comes from `.ai/agents/`. The generator localizes the same Claude dialect (worktrees/subagents/TodoWrite → `delegate_task` with per-role `toolsets` + the todo tool + serialize edits, and the `/worktrees` spawn CLI → `hermes chat -s hephaestus/<skill> -q`, since `-q` bypasses the slash dispatcher and a bare `/<skill>` would reach the model as literal text) and warns that a delegate inherits none of the parent conversation and a cwd frozen at session launch, not absent (measured via `HERMES_DUMP_REQUESTS`; details in the #175 fragment and the test comment in `tests/test_hermes_adapters.sh`). Hermes has no project-local skill discovery, so the files stay inert until the project's `.hermes/skills` is wired — `skills.external_dirs` (recommended) or `HERMES_HOME` — `bash scripts/verify-hermes-load.sh` checks that wiring against a live install; its default checks are free (layout plus unbilled CLI calls) and so prove a skill resolves but not that its body loads, which `--live` confirms with one billed probe. Run `./scripts/sync-hermes-adapters.sh` after workflow or agent edits and `--check` in quality gates.
- **Cursor adapters are generated too**: `.cursor/commands/` comes from `.ai/workflows/`; `.cursor/agents/` comes from `.ai/agents/`; `.cursor/rules/hephaestus.mdc` is derived from both. Cursor mechanics that shape the output: command files are injected **verbatim** (frontmatter is stripped only for imported `.claude/commands`), so they carry no YAML and lead with the workflow's opening line — Cursor's hover preview; `$ARGUMENTS`/`$1`… *are* substituted; frontmatter values are not unquoted, so they are emitted bare; `readonly: true` is honoured and emitted for agents with no shell and no edit tools, while `tools:` is only rendered to the model as prose and is never an access control. The `/worktrees` spawn CLI localizes to `cursor-agent -p`; the `-p` is load-bearing, since the slash dispatcher is bound to the TUI input widget and a bare positional `/start-issue <N>` reaches the model as literal text instead of dispatching. Cursor spawns are therefore headless — `-p` prints and exits — deliberately unlike the Claude and Codex rows. Run `./scripts/sync-cursor-adapters.sh` after workflow or agent edits and `--check` in quality gates.
- **loop.sh's unattended flags are drift-guarded at both levels** (#192). Each `verify-*-load.sh` asserts the approval/headless flags its harness's dispatch entry depends on — `codex exec --help` checked separately from the top level, since the flag sets differ — and `loop.sh` preflights its own dispatch table once at startup: every flag in the entry's argv must appear in the harness's help (the subcommand's help when the entry leads with one), else it exits before taking the lock. A renamed approval flag otherwise hangs the session on a prompt no TTY can answer — worse than a missing binary, which fails loudly. The flags are derived from `HARNESS_ARGV`, so there is no second list to drift; help text is the honest ceiling (cf. the config-parsing preflight rejected in #191), and `HEPH_NO_PREFLIGHT=1` waves through a flag that works but is undocumented.
...
- **`/finish` cleanup tests execute real Git operations** from `tests/fixtures/finish_cleanup_recipes.sh` in temporary repositories, including branch-tip races, the return to a stash's original checkout, and stash conflicts. Contract assertions in `tests/test_finish_cleanup.sh` pin that executable model to the canonical workflow's safety decisions; update both when cleanup semantics change. Linked worktrees defer cleanup to a primary session; no finish path removes a worktree or restores another checkout's stash.
...
Five agent roles, canonical in `.ai/agents/` and stratified by least-privilege tool access. Coder is the only agent that can modify files (runs in `isolation: worktree` for safe parallel edits). Reviewer, tester, and explorer are read-only. Researcher has web access but no shell. The frontmatter schema is Claude dialect (`tools:`, `model:`, `isolation:`) kept as the spec schema — the tiers are already neutral and `isolation` is a concept, not a Claude API; a second vocabulary would be bloat.
...
Nothing in `scripts/`, `install.sh`, `update.sh`, or `uninstall.sh` reads CLAUDE.md as config, and `models.conf` has exactly one reader. Keep it that way. `install.sh --interactive` edits CLAUDE.md sections at the operator's dictation — writing the surface a human asked it to fill is not consumption; it reads CLAUDE.md only to detect whether a section exists, never to obtain a value.
...
- Edit canonical workflow specs in `.ai/workflows/` and agent definitions in `.ai/agents/`, not the generated `.claude/commands/` or `.claude/agents/` adapters; regenerate those with `./scripts/sync-agent-adapters.sh`.
