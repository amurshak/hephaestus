...
- **`/finish` cleanup tests execute the canonical shell blocks** in temporary repositories (`tests/test_finish_cleanup.sh`), including branch-tip races, the return to a stash's original checkout, and stash conflicts. Keep the `task-branch-cleanup`, `task-stash-capture`, `task-stash-checkout`, and `task-stash-restore` markers stable so tests exercise shipped operations. Linked worktrees defer cleanup to a primary session; no finish path removes a worktree or restores another checkout's stash.
